...
class DiagnosticStore:
        ...
        try:
            with self._lock:
                ...
                conn.execute(
                    ...
                    (
                        _utc_now(), time.monotonic_ns(), level.upper()[:16],
    ...
    def query(self, query: DiagnosticQuery = DiagnosticQuery()) -> list[dict[str, Any]]:
        ...
        if query.text:
            clauses.append("(message LIKE ? ESCAPE '\\' OR metadata_json LIKE ? ESCAPE '\\')")
    ...
    def operation_summaries(
        self, *, subsystem: str = "savesync", page: int = 1, page_size: int = 20
    ) -> list[dict[str, Any]]:
        """Return a bounded higher-level operation view for Maintenance."""
        size = max(1, min(int(page_size), 50))
        offset = (max(1, int(page)) - 1) * size
        try:
            with self._lock:
                starts = self._connect().execute(
                    """SELECT operation_id, timestamp_utc, message, metadata_json
                    FROM diagnostic_events
                    WHERE subsystem = ? AND event_code = 'operation.started'
                          AND operation_id IS NOT NULL
                    ORDER BY id DESC LIMIT ? OFFSET ?""",
                    (subsystem, size, offset),
                ).fetchall()
        except Exception:
        ...
        for start in starts:
            chain = self.operation_chain(str(start["operation_id"]), page_size=200)
            try:
                start_metadata = json.loads(start["metadata_json"])
            except (ValueError, TypeError):
                start_metadata = {}
            result_event = next(
                (
                    item for item in reversed(chain)
                    if item["event_code"] in {"operation.result", "operation.failed"}
                ),
                chain[-1] if chain else {},
            )
            result_metadata = result_event.get("metadata", {})
            summaries.append(
                {
                    "operation_id": start["operation_id"],
                    "timestamp_utc": start["timestamp_utc"],
                    "name": start_metadata.get("operation_name") or start["message"],
                    "source": start_metadata.get("source"),
                    "effective_mode": start_metadata.get("effective_mode"),
                    "status": result_metadata.get(
                        "status",
                        "failed" if result_event.get("event_code") == "operation.failed" else "success",
                    ),
                    "reason": result_metadata.get("reason"),
                    "generation": result_metadata.get("generation"),
                    "cursor_before": result_metadata.get("cursor_before"),
                    "cursor_after": result_metadata.get("cursor_after"),
                    "uploaded": result_metadata.get("uploaded", 0),
                    "downloaded": result_metadata.get("downloaded", 0),
                    "conflicts": result_metadata.get("conflicts", 0),
                    "unchanged": result_metadata.get("unchanged", 0),
