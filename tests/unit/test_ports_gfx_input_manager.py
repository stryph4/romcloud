...
class TestKeyboardRouting:
    ...
    def test_enter_routes_as_text_submit_while_osk_is_active(self, tmp_path: Path):
        manager = _manager(tmp_path)
        pygame = manager._pygame
        ...
        assert result == InputEvent(action=Action.TEXT_SUBMIT, source="keyboard")
...
class TestControllerRouting:
    ...
    def test_start_and_l3_keep_controller_source_for_osk_shortcuts(
        self, tmp_path: Path
    ):
        joystick = FakeJoystick(guid="g1")
        manager = _manager(
        ...
        pygame = manager._pygame
        manager.handle_event(
            FakeEvent(type=pygame.CONTROLLERDEVICEADDED, device_index=0),
            screen_w=1920,
            screen_h=1080,
        )

        start = manager.handle_event(
            FakeEvent(
        ...
        assert start == InputEvent(action=Action.MENU, source="controller")
        assert l3 == InputEvent(
            action=Action.TEXT_TOGGLE_SHIFT, source="controller"
        )
