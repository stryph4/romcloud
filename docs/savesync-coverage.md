...
| PS1 | DuckStation | `/userdata/saves/duckstation/memcards/**` and root `*.sav` | Existing, confirmed | UT | Per physical card; commercial-game domains inside supported valid 128 KiB raw cards merged automatically within a card; opaque fallback | UT | UT | UT for DuckStation | Automatic `ps1-raw-memory-card`; opaque fallback | Yes, especially card adapter | Resume files excluded; invalid, unsupported, or ambiguous cards stay opaque |
| PS2 | PCSX2 | `/userdata/saves/ps2/pcsx2/Mcd*.ps2`, folder cards, `sstates/**`; legacy `pcsx2` roots retained | Existing, confirmed | UT | Per physical card slot for monolithic `.ps2` images (each Mcd*.ps2 is an independent conflict unit, kept opaque internally); per folder card for PCSX2 Folder Memory Cards; states deliberately broad | UT | UT | UT for PS2/PCSX2 | Monolithic `.ps2` opaque; automatic folder adapter with opaque fallback | Yes | Multi-entry folder cards remain opaque unless complete grouping is proven |
